...
class MLPPolicySL(BasePolicy, nn.Module, metaclass=abc.ABCMeta):
    ...
    def forward(self, observation: torch.FloatTensor) -> Any:
        ...
        x = self.mean_net(observation)
        return distributions.Normal(x, torch.exp(self.logstd))
    ...
    def update(self, observations, actions):
        ...
        predicts = self.forward(ptu.from_numpy(observations))
        # loss = F.mse_loss(predicts, ptu.from_numpy(actions))
        loss = -predicts.log_prob(ptu.from_numpy(actions)).sum()
