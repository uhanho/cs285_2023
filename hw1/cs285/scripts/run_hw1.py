    ...
    for itr in range(params['n_iter']):
        ...
        if itr == 0:
            ...
            if params['do_dagger']:
                ...
                for path in paths:
                  action_tmp = expert_policy.get_action(path["observation"])
                  path["action"] = action_tmp
        ...
        for _ in range(params['num_agent_train_steps_per_iter']):
          ...
          # ob_data = replay_buffer.obs[:params['train_batch_size']]
          # ac_data = replay_buffer.acs[:params['train_batch_size']]
          # ob_batch, ac_batch = np.random.permutation(ob_data), np.random.permutation(ac_data)
          batch_indices = np.random.permutation(len(replay_buffer.obs))[:params['train_batch_size']]
